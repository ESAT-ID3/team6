...
const getSpendInfoPerCategory = (
  banks_info: BankAccountDetails[],
  date: string
) => {
  let labels: string[] = [];
  let data: number[] = [];
  banks_info.forEach((bank) => {
    let transactions = bank.data.filter(
...
async function getCurrentMonthData(userId: string | undefined) {
  ...
  if (bankInfo) {
    ...
    monthSpend.forEach((transaction) => {
      let pos = result.findIndex(
        (item) => item.category === transaction.category
      );
      if (pos === -1) {
        result.push({
          category: transaction.category,
          spend: Math.abs(transaction.amount),
        });
      } else {
...
async function storeCurrentBudget(
...
const getTotalBalance = async (userId: string | undefined): Promise<number> => {
  if (!userId) return 0;
